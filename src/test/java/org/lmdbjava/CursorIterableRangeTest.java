...
public final class CursorIterableRangeTest {
  ...
  void testSignedComparator(final String keyType, final String startKey, final String stopKey, final String expectedKV) {
    testCSV(ByteBuffer::compareTo,
            true,
  ...
  void testUnsignedComparator(final String keyType, final String startKey, final String stopKey, final String expectedKV) {
    testCSV(AbstractByteBufferProxy::compareBuff,
            false,
            createBasicDBPopulator(),
            EnumSet.of(MDB_CREATE),
            keyType,
  ...
  void testSignedComparatorDupsort(final String keyType, final String startKey, final String stopKey, final String expectedKV) {
    testCSV(ByteBuffer::compareTo,
            true,
  ...
  void testUnsignedComparatorDupsort(final String keyType, final String startKey, final String stopKey, final String expectedKV) {
    testCSV(AbstractByteBufferProxy::compareBuff,
            false,
            createMultiDBPopulator(2),
            EnumSet.of(MDB_CREATE, MDB_DUPSORT),
  ...
  void testIntegerKey(final String keyType, final String startKey, final String stopKey, final String expectedKV) {
    testCSV(AbstractByteBufferProxy::compareBuff,
            false,
  ...
  void testLongKey(final String keyType, final String startKey, final String stopKey, final String expectedKV) {
    testCSV(AbstractByteBufferProxy::compareBuff,
            false,
  ...
  private void testCSV(final Comparator<ByteBuffer> comparator,
    ...
    testCSV(comparator, nativeCb, dbPopulator, flags, keyType, startKey, stopKey, expectedKV, Integer.BYTES, ByteOrder.BIG_ENDIAN);
  }
  ...
  private void testCSV(final Comparator<ByteBuffer> comparator,
    ...
    FileUtil.useTempFile(file -> {
      try (final Env<ByteBuffer> env =
        ...
        final Dbi<ByteBuffer> dbi = env.openDbi(DB_1, comparator, nativeCb, flags.toArray(new DbiFlags[0]));
...
//      assertThat(act).withFailMessage("Files are not equal").isEqualTo(exp);
  ...
  private ByteBuffer parseKey(final String key,
                              final int keyLen,
                              final ByteOrder byteOrder) {
    if (key != null) {
  ...
  private long getLong(final ByteBuffer byteBuffer,
                       final ByteOrder byteOrder) {
