...
public class EnvFlagSetTest {
  ...
  public void testEmpty() {
    ...
    final EnvFlagSet envFlagSet2 = EnvFlagSet.builder().build();
    assertThat(envFlagSet).isEqualTo(envFlagSet2);
    assertThat(envFlagSet).isNotEqualTo(EnvFlagSet.of(EnvFlags.MDB_FIXEDMAP));
    assertThat(envFlagSet)
        .isNotEqualTo(EnvFlagSet.of(EnvFlags.MDB_FIXEDMAP, EnvFlags.MDB_NORDAHEAD));
    assertThat(envFlagSet)
        .isNotEqualTo(
            EnvFlagSet.builder()
  ...
  public void testOf() {
    ...
    final EnvFlagSet envFlagSet2 = EnvFlagSet.builder().setFlag(envFlag).build();
    assertThat(envFlagSet).isEqualTo(envFlagSet2);
  }
  ...
  public void testBuilder() {
    final EnvFlags envFlag1 = EnvFlags.MDB_FIXEDMAP;
    final EnvFlags envFlag2 = EnvFlags.MDB_NORDAHEAD;
    final EnvFlagSet envFlagSet = EnvFlagSet.builder().setFlag(envFlag1).setFlag(envFlag2).build();
    assertThat(envFlagSet.getMask()).isEqualTo(MaskedFlag.mask(envFlag1, envFlag2));
    assertThat(envFlagSet.size()).isEqualTo(2);
    ...
    final EnvFlagSet envFlagSet2 = EnvFlagSet.builder().withFlags(envFlag1, envFlag2).build();
    final EnvFlagSet envFlagSet3 =
        EnvFlagSet.builder().withFlags(new HashSet<>(Arrays.asList(envFlag1, envFlag2))).build();
    assertThat(envFlagSet).isEqualTo(envFlagSet2);
